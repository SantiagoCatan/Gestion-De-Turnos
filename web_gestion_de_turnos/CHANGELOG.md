...
- Cree un proyecto de React con create-vite: `npm init --yes vite@latest "web_gestion_de_turnos" -- --template=react-ts  --no-rolldown --no-immediate --eslint`.
- Actualizando `vite.config.ts` con puerto.
- Crear archivo de proyecto (`web_gestion_de_turnos.esproj`).
